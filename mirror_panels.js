...
console.log('Backup saved:', (h.length/1024).toFixed(1), 'KB');

// Remove ALL old panel CSS blocks we added (everything between our markers)
const patterns = [
  /\/\* === 4 CFD PANELS[\s\S]*?@media\(max-width:480px\)\{[\s\S]*?\}\n/g,
  /\/\* === EXNESS SIDEBAR[\s\S]*?\}\n/g,
  /\n#p-commodities [^{]*\{[^}]*\}/g,
  /\n#p-stocks [^{]*\{[^}]*\}/g,
  /\n#p-indices [^{]*\{[^}]*\}/g,
...
console.log('Cleaned old CSS');

const css = `
/* === CFD PANELS MIRROR FOREX === */
/* Positions match forex exactly */
...
#p-commodities .ctop .ci,#p-stocks .ctop .ci,#p-indices .ctop .ci,#p-crypto .ctop .ci{
  ...
  filter:drop-shadow(0 0 28px rgba(0,229,255,.55)) drop-shadow(0 0 60px rgba(0,180,216,.35))}
